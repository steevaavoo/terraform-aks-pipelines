...
From the Multi-Container Registry Tutorial, I gather that we will need (in addition to the existing resources
...
since we have one of those defined... I've changed entries to correspond to my naming conventions. I also removed
"georeplication_locations" and set "sku" to "Basic" as this is a demo.
...
For now, we're not going to use the ACR - that mandates building an Image ourselves. This will be the next task.

Meanwhile, we will ~steal~ create some YAML files to create a Deployment and Service for the nginxdemo and customise them accordingly.

Then we added the tasks in Pipeline to copy YAMLS to the Artifact Drop directory.
